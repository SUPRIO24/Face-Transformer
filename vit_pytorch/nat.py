...
# Used the Neighbourhood Attention Transformer code from https://github.com/SHI-Labs/Neighborhood-Attention-Transformer/blob/main/classification/nat.py
...
class NAT(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        """
        The build of NAT is such that it produces the output in a vector of size NUM_ClASS,
        thus the self.head creates the vector of output in size NUM_CLASS.

        While the backbone is in training mode, the output is preduced in NUM_CLASS which gets 
        compared with the actual labels that is provided. 
        
        But when the backbone is used for evaluation over the testing datasets, then an embedding 
        vector is created to store the output vector. But as keeping a large embedding_size leads to
        space limit exceeded error, I created another head, which would return the vector in a smaller
        size. 
        """
    ...
    """
    Added a boolean value embed_flag which is default to be False, while performing evaluation on the testing dataset, call the head1 function else while training the head function.
    """

    def forward(self, x, embed_flag=False):
